...
def main():
    if len(sys.argv) < 2:
        print(f"At least one arg is required. Usage: miro_uploader <destination_path> [monitored_path]")
        exit(1)
    ...
    event_handler = RegexMatchingEventHandler(regexes=[r"^[\x00-\x7F]*jpg$"], ignore_directories=True, case_sensitive=False)
